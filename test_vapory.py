...
plt.subplot(1,3,3)
plt.imshow((0.5*(image_right-image_left)+127)/255)

if False:
    scene_left  = vapory.Scene( camera_left, objects= objects,global_settings=[radiosity],included = ["colors.inc", "textures.inc"],)
    scene_right = vapory.Scene( camera_right, objects= objects,global_settings=[radiosity],included = ["colors.inc", "textures.inc"],)
    print str(scene_left)
    image_left  = scene_left.render ( width=300, height=200,antialiasing = 0.01,quality=10)
    image_right = scene_right.render( width=300, height=200,antialiasing = 0.01,quality=10)
    plt.ion()
    plt.figure()
    plt.subplot(1,3,1)
    plt.imshow(image_left)
...
plt.imshow(image_left2)
imsave("image_left2.png",image_left2)
print "done"
